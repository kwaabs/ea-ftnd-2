...
interface DistrictBoundaryAggregateParams {
  dateFrom?: string
  dateTo?: string
  regions?: string[]
  districts?: string[]
...
interface DistrictBoundaryDataPoint {
...
interface DistrictBoundaryDailyRecord {
  meter_number: string
...
interface ProcessedBoundaryData {
  ...
  rawData: DistrictBoundaryDataPoint[]
}
...
function buildQueryString(params: DistrictBoundaryAggregateParams): string {
...
function buildDailyQueryString(params: DistrictBoundaryAggregateParams): string {
  const queryParams = new URLSearchParams()
...
function processDistrictBoundaryData(data: DistrictBoundaryDataPoint[]): ProcessedBoundaryData {
...
export function useDistrictBoundaryAggregate(params: DistrictBoundaryAggregateParams) {
  const queryString = buildQueryString(params)

  return useQuery<ProcessedBoundaryData>({
    queryKey: [
      "district-boundary-aggregate",
      params.dateFrom,
      params.dateTo,
      params.regions,
      params.districts,
    ...
    queryFn: async () => {
      const url = `${API_BASE_URL}/api/v1/meters/consumption/aggregate/district?${queryString}`
      const response = await fetch(url)
      if (!response.ok) {
        throw new Error("Failed to fetch district boundary data")
      }
      const data = await response.json()
      return processDistrictBoundaryData(data)
    },
    enabled: !!params.dateFrom && !!params.dateTo,
  })
}

export function useDistrictBoundaryDaily(params: DistrictBoundaryAggregateParams) {
  const queryString = buildDailyQueryString(params)

  return useQuery<DistrictBoundaryDailyRecord[]>({
    queryKey: [
      "district-boundary-daily",
      params.dateFrom,
      params.dateTo,
      params.regions,
      params.districts,
    ...
    queryFn: async () => {
      const url = `${API_BASE_URL}/api/v1/meters/consumption/daily/district?${queryString}`
      console.log("[v0] District Boundary Daily URL:", url)
      const response = await fetch(url)
      if (!response.ok) {
        throw new Error("Failed to fetch district boundary daily data")
      }
      const data = await response.json()
      console.log("[v0] District Boundary Daily Data:", {
...
export function useDistrictBoundaryMeterStatusCounts(params: DistrictBoundaryAggregateParams) {
  const queryParams = new URLSearchParams()

  if (params.dateFrom) queryParams.append("dateFrom", params.dateFrom)
  if (params.dateTo) queryParams.append("dateTo", params.dateTo)

  queryParams.append("meterType", "DISTRICT_BOUNDARY")
  ...
  return useQuery<MeterStatusCounts>({
    queryKey: [
      "district-boundary-meter-status-counts",
      params.dateFrom,
      params.dateTo,
      params.regions,
      params.districts,
    ...
    queryFn: async () => {
      ...
      const response = await fetch(url)
      if (!response.ok) {
        throw new Error("Failed to fetch district boundary meter status counts")
      }
      const data = await response.json()
      return data
    },
...
export function useDistrictBoundaryMeterStatus(params: DistrictBoundaryAggregateParams) {
  const queryParams = new URLSearchParams()
  ...
  return useQuery<MeterStatusRecord[]>({
    queryKey: [
      "district-boundary-meter-status",
      params.dateFrom,
      params.dateTo,
      params.regions,
      params.districts,
    ...
    queryFn: async () => {
      ...
      const response = await fetch(url)
      if (!response.ok) {
        throw new Error("Failed to fetch district boundary meter status")
      }
      const data = await response.json()
      return data
    },
