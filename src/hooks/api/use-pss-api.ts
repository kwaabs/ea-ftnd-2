...
interface PssDataPoint {
  system_name: "import_kwh" | "export_kwh"
  region: string
  station: string
  feeder_panel_name: string
  ic_og: "IC" | "OG" // Added ic_og field
  active_meters: number
  total_meter_count: number
  all_meters_count: number // Added all_meters_count field
  meter_type: string
  group_period: string
  total_consumption: number
}

interface PssDailyRecord {
  meter_number: string
  ...
  station?: string
  feeder_panel_name?: string
  ic_og?: "IC" | "OG" // Added ic_og field to daily records
  voltage_kv?: string
  meter_type: string
}

interface ProcessedPssData {
  ...
  heatmapData: {
    ...
    dailySupply: {
      date: string
      supply: number
      reverseFlow: number // Added reverseFlow to heatmap data structure
    }[]
  }[]
  rawData: PssDataPoint[]
}

function buildQueryString(params: PssAggregateParams): string {
...
function buildDailyQueryString(params: PssAggregateParams): string {
  const queryParams = new URLSearchParams()
...
function processPssData(data: PssDataPoint[]): ProcessedPssData {
...
export function usePssAggregate(params: PssAggregateParams) {
  const queryString = buildQueryString(params)

  return useQuery<ProcessedPssData>({
    queryKey: [
      "pss-aggregate",
      params.dateFrom,
      params.dateTo,
    ...
    queryFn: async () => {
      const url = `${API_BASE_URL}/api/v1/meters/consumption/aggregate/pss?${queryString}`
      const response = await fetch(url)
      if (!response.ok) {
        throw new Error("Failed to fetch PSS data")
      }
      const data = await response.json()
      return processPssData(data)
    },
    enabled: !!params.dateFrom && !!params.dateTo,
  })
}

export function usePssDaily(params: PssAggregateParams) {
  const queryString = buildDailyQueryString(params)

  return useQuery<PssDailyRecord[]>({
    queryKey: [
      "pss-daily",
      params.dateFrom,
      params.dateTo,
    ...
    queryFn: async () => {
      const url = `${API_BASE_URL}/api/v1/meters/consumption/daily/pss?${queryString}`
      const response = await fetch(url)
      if (!response.ok) {
        throw new Error("Failed to fetch PSS daily data")
      }
      const data = await response.json()
      return data
    },
