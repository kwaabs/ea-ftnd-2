...
export function RegionMiniMap({ regionName, districtGeometries = [], meterCoordinates = [] }: RegionMiniMapProps) {
    ...
    useEffect(() => {
        ...
        map.current.on("load", () => {
            ...
            districtGeometries.forEach((district, index) => {
                if (!district.geometry || district.geometry.type !== "Polygon") return
                ...
                const normalizedConsumption = district.consumption && maxConsumption > 0
                ...
                // Add hover effects with temporary popup
                map.current!.on("mouseenter", `district-fill-${index}`, (e) => {
                    if (!map.current) return
                    ...
                    popupRef.current = new maplibregl.Popup({
                        ...
                        .setHTML(
                            `<div style="padding: 6px 10px;">
                                <strong style="font-size: 13px;">${district.district}</strong>
                            </div>`
                        )
                        .addTo(map.current)
                })
            ...
            const validMeters = meterCoordinates.filter(meter => {
                return typeof meter.lat === 'number' &&
    ...
    const boundaryCount = meterCoordinates.filter(m => m.type === 'REGIONAL_BOUNDARY' || m.type === 'DISTRICT_BOUNDARY').length
        ...
        <Card className="h-full flex flex-col">
            ...
            <CardContent className="p-0 flex-1 flex flex-col">
                {districtGeometries.length === 0 ? (
                    ...
                    <>
                        <div ref={mapContainer} className="flex-1 w-full min-h-[450px]" />
