...
import { HolleyConsumptionDetailTable } from "@/components/customer-sales/holley-consumption-detail"
import { normalizeRegionName, shortRegionLabel } from "@/hooks/use-resolved-region-name"
import { cn } from "@/lib/utils"

// Teal, distinct from Zeus (blue), MMS (green), BOT (amber), BXC (purple), and PNS (rose) elsewhere on this page.
const HOLLEY_COLOR = "#0d9488"
...
interface HolleyConsumptionViewProps {
  dateRange: { start: string; end: string }
}

export function HolleyConsumptionView({ dateRange }: HolleyConsumptionViewProps) {
  ...
  const { data: regionAgg = [], isLoading: regionLoading } = useHolleyConsumptionAggregate({
  ...
  // Merge on the same normalized key used everywhere else on this page, as
  // a client-side safety net against casing/whitespace drift in the source
  // data (same reasoning as botconsumption's byRegion, minus the
  // server-side trim() this table's region column doesn't need — it's a
  // plain varchar, not blank-padded bpchar).
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold tracking-tight text-foreground">HOLLEY</h3>
        <p className="text-muted-foreground mt-1 text-sm">
          Holley-ingested consumption readings — a legacy meter source, independent of Zeus, MMS, BOT, BXC and PNS
          {selectedRegion ? (
            <span className="text-teal-700"> · filtered by {shortRegionLabel(selectedRegion)}</span>
          ) : null}
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardContent className="pt-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">Holley-ingested readings</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            ...
            {regionLoading ? (
              ...
              <p className="text-2xl font-bold text-teal-700 tabular-nums">{byRegion.length}</p>
            )}
            <p className="text-xs text-muted-foreground mt-1">With holley-ingested data</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          {regionLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-12 text-center">
              No holley-consumption data for this period.
            </p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-right py-2 px-4 font-medium text-teal-700">Consumption</th>
      ...
      <HolleyConsumptionDetailTable dateRange={dateRange} region={effectiveRegion} />
    </div>
  )
}
