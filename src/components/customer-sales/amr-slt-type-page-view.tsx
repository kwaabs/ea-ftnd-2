...
function formatSltLabel(raw: string) {
  return raw.replace(/_/g, " ")
}

export function AmrSltTypePageView({ sltType }: { sltType: string }) {
    ...
    <div className="space-y-6">
      <div className="flex items-start gap-4">
        <Button variant="outline" size="sm" asChild className="mt-1 shrink-0">
          <Link href="/customer-sales/postpaid/amr">
        ...
        <div>
          <h2 className="text-3xl font-semibold tracking-tight text-foreground">
            AMR — {formatSltLabel(sltType)}
          </h2>
          <p className="text-muted-foreground mt-1">
            Daily AMR meters and status for SLT type{" "}
            <span className="text-orange-700 font-medium">
              {formatSltLabel(sltType)}
            </span>
          </p>
        </div>
      </div>
