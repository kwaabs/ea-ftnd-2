...
import { BxcConsumptionDetailTable } from "@/components/customer-sales/bxc-consumption-detail"
import { normalizeRegionName, shortRegionLabel } from "@/hooks/use-resolved-region-name"
import { cn } from "@/lib/utils"

// Purple, distinct from Zeus (blue), MMS (green), and BOT (amber) elsewhere on this page.
const BXC_COLOR = "#9333ea"
...
interface BxcConsumptionViewProps {
  dateRange: { start: string; end: string }
}

export function BxcConsumptionView({ dateRange }: BxcConsumptionViewProps) {
  ...
  const { data: regionAgg = [], isLoading: regionLoading } = useBxcConsumptionAggregate({
  ...
  // region is a plain varchar(10) here (no bpchar padding to worry about,
  // unlike botconsumption), but GROUP BY still isn't case-folded, so two
  // rows differing only in casing would land as separate groups if the
  // source data is ever inconsistent — merge on the same normalized key
  // used everywhere else on this page as a cheap client-side safety net.
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold tracking-tight text-foreground">BXC</h3>
        <p className="text-muted-foreground mt-1 text-sm">
          Bot-ingested consumption readings — a legacy meter source, independent of Zeus, MMS and BOT
          {selectedRegion ? (
            <span className="text-purple-700"> · filtered by {shortRegionLabel(selectedRegion)}</span>
          ) : null}
        </p>
      </div>
        ...
        <Card>
          <CardContent className="pt-5">
            ...
            {regionLoading ? (
              ...
              <p className="text-2xl font-bold text-purple-700 tabular-nums">{byRegion.length}</p>
            )}
        ...
        <CardContent>
          {regionLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-12 text-center">
              No bxc-consumption data for this period.
            </p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-right py-2 px-4 font-medium text-purple-700">Consumption</th>
      ...
      <BxcConsumptionDetailTable dateRange={dateRange} region={effectiveRegion} />
    </div>
  )
}
