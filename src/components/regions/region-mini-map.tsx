"use client";

import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { MapPin } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import maplibregl from "maplibre-gl";
import "maplibre-gl/dist/maplibre-gl.css";

interface RegionMiniMapProps {
  regionName: string;
  districtGeometries: Array<{
    district: string;
    geometry: any; // GeoJSON geometry
    consumption?: number;
  }>;
  meterCoordinates?: Array<{
    lat: number;
    lng: number;
    type: string;
    meter_number: string;
    brand?: string;
    station?: string;
    boundary_metering_point?: string;
  }>;
}

export function RegionMiniMap({
  regionName,
  districtGeometries = [],
  meterCoordinates = [],
}: RegionMiniMapProps) {
  const mapContainer = useRef<HTMLDivElement>(null);
  const map = useRef<maplibregl.Map | null>(null);
  const markersRef = useRef<maplibregl.Marker[]>([]);
  const popupRef = useRef<maplibregl.Popup | null>(null);
  const [hoveredDistrict, setHoveredDistrict] = useState<{name: string, consumption?: number} | null>(null);

  useEffect(() => {
    if (!mapContainer.current || districtGeometries.length === 0) return;
    if (map.current) return; // Already initialized

    console.log("=== RegionMiniMap Initialization ===");
    console.log("Total meter coordinates:", meterCoordinates.length);

    // Calculate initial center from all geometries
    const bounds = new maplibregl.LngLatBounds();
    districtGeometries.forEach((district) => {
      if (district.geometry && district.geometry.type === "Polygon") {
        district.geometry.coordinates[0].forEach((coord: number[]) => {
          bounds.extend(coord as [number, number]);
        });
      }
    });

    const center = bounds.getCenter();

    // Initialize map with CartoDB basemap
    map.current = new maplibregl.Map({
      container: mapContainer.current,
      style: "https://basemaps.cartocdn.com/gl/voyager-gl-style/style.json",
      center: [center.lng, center.lat],
      zoom: 10,
      attributionControl: false,
    });

    // Add navigation controls
    map.current.addControl(new maplibregl.NavigationControl(), "top-right");

    map.current.on("load", () => {
      if (!map.current) return;

      console.log(">>> Map loaded, adding district layers");

      const maxConsumption = Math.max(
        ...districtGeometries.map((d) => d.consumption || 0),
      );

      districtGeometries.forEach((district, index) => {
        if (!district.geometry) return;
        if (district.geometry.type !== "Polygon" && district.geometry.type !== "MultiPolygon") {
          console.log(`[v0] Skipping district ${district.district} with unsupported geometry type: ${district.geometry.type}`);
          return;
        }

        map.current!.addSource(`district-${index}`, {
          type: "geojson",
          data: {
            type: "Feature",
            properties: {
              name: district.district,
              consumption: district.consumption || 0,
            },
            geometry: district.geometry,
          },
        });

        const normalizedConsumption =
          district.consumption && maxConsumption > 0
            ? district.consumption / maxConsumption
            : 0.3;

        map.current!.addLayer({
          id: `district-fill-${index}`,
          type: "fill",
          source: `district-${index}`,
          paint: {
            "fill-color": "#3b82f6",
            "fill-opacity": normalizedConsumption * 0.6 + 0.1,
          },
        });

        map.current!.addLayer({
          id: `district-border-${index}`,
          type: "line",
          source: `district-${index}`,
          paint: {
            "line-color": "#2563eb",
            "line-width": 2,
          },
        });

        // Add hover effects with state display
        map.current!.on("mouseenter", `district-fill-${index}`, () => {
          if (map.current) {
            map.current.getCanvas().style.cursor = "pointer";
          }
          setHoveredDistrict({
            name: district.district,
            consumption: district.consumption
          });
        });

        map.current!.on("mouseleave", `district-fill-${index}`, () => {
          if (map.current) {
            map.current.getCanvas().style.cursor = "";
          }
          setHoveredDistrict(null);
        });
      });

      // Add meters
      console.log(">>> Adding meter markers");

      const validMeters = meterCoordinates.filter((meter) => {
        return (
          typeof meter.lat === "number" &&
          typeof meter.lng === "number" &&
          !isNaN(meter.lat) &&
          !isNaN(meter.lng) &&
          meter.lat !== 0 &&
          meter.lng !== 0
        );
      });

      console.log("Valid meters after filtering:", validMeters.length);

      if (validMeters.length > 0) {
        console.log("[v0] Starting meter grouping...");
        console.log("[v0] Meter types in validMeters:", {
          BSP: validMeters.filter(m => m.type === "BSP").length,
          DTX: validMeters.filter(m => m.type === "DTX").length,
          REGIONAL_BOUNDARY: validMeters.filter(m => m.type === "REGIONAL_BOUNDARY").length,
          DISTRICT_BOUNDARY: validMeters.filter(m => m.type === "DISTRICT_BOUNDARY").length,
          Other: validMeters.filter(m => !["BSP", "DTX", "REGIONAL_BOUNDARY", "DISTRICT_BOUNDARY"].includes(m.type)).length,
        });

        const bspGroups: Record<string, typeof validMeters> = {};
        const individualMeters: typeof validMeters = [];

        validMeters.forEach((meter, idx) => {
          if (idx < 5) {
            console.log(`[v0] Meter ${idx + 1}: type="${meter.type}", station="${meter.station}", meter_number="${meter.meter_number}"`);
          }
          
          if (meter.type === "BSP" && meter.station) {
            const key = `bsp-${meter.station}`;
            if (!bspGroups[key]) {
              bspGroups[key] = [];
            }
            bspGroups[key].push(meter);
            if (Object.keys(bspGroups).length <= 3) {
              console.log(`[v0] Grouped BSP meter ${meter.meter_number} into station group: ${meter.station}`);
            }
          } else {
            individualMeters.push(meter);
            if (individualMeters.length <= 5) {
              console.log(`[v0] Added to individualMeters: type="${meter.type}", meter_number="${meter.meter_number}"`);
            }
          }
        });

        console.log("[v0] Grouping complete:");
        console.log("[v0] BSP station groups:", Object.keys(bspGroups).length);
        console.log("[v0] Individual meters:", individualMeters.length);
        console.log("[v0] Individual meters by type:", {
          BSP: individualMeters.filter(m => m.type === "BSP").length,
          DTX: individualMeters.filter(m => m.type === "DTX").length,
          REGIONAL_BOUNDARY: individualMeters.filter(m => m.type === "REGIONAL_BOUNDARY").length,
          DISTRICT_BOUNDARY: individualMeters.filter(m => m.type === "DISTRICT_BOUNDARY").length,
        });

        // Add BSP station markers (grouped)
        Object.entries(bspGroups).forEach(([key, meters]) => {
          if (!map.current || meters.length === 0) return;

          const firstMeter = meters[0];
          const el = document.createElement("div");
          el.style.cssText = "width: 36px; height: 36px; cursor: pointer;";

          const inner = document.createElement("div");
          inner.style.cssText = `
                        width: 100%;
                        height: 100%;
                        background-color: #3b82f6;
                        border: 3px solid white;
                        border-radius: 50%;
                        display: flex;
                        align-items: center;
                        justify-content: center;
                        color: white;
                        font-weight: bold;
                        font-size: 12px;
                        box-shadow: 0 2px 8px rgba(0,0,0,0.3);
                        transition: transform 0.2s ease;
                    `;
          inner.textContent = meters.length.toString();
          el.appendChild(inner);

          el.addEventListener("mouseenter", () => {
            inner.style.transform = "scale(1.15)";
            inner.style.boxShadow = "0 4px 12px rgba(0,0,0,0.4)";
          });

          el.addEventListener("mouseleave", () => {
            inner.style.transform = "scale(1)";
            inner.style.boxShadow = "0 2px 8px rgba(0,0,0,0.3)";
          });

          const metersList = meters.map((m) => m.meter_number).join("<br/>");
          const stationName = firstMeter.station || "Unknown Station";

          const marker = new maplibregl.Marker({
            element: el,
            anchor: "center",
          })
            .setLngLat([firstMeter.lng, firstMeter.lat])
            .addTo(map.current);

          // Add click handler to show shared popup
          el.addEventListener("click", (e) => {
            e.stopPropagation();
            const mapInstance = map.current;
            if (!mapInstance) return;
            
            // Remove existing popup if any
            if (popupRef.current) {
              popupRef.current.remove();
            }

            // Create new popup at this marker's location
            popupRef.current = new maplibregl.Popup({ 
              offset: 15,
              closeButton: true,
              closeOnClick: true 
            })
              .setLngLat([firstMeter.lng, firstMeter.lat])
              .setHTML(
                `<div style="padding: 8px; max-width: 200px;">
                  <strong style="font-size: 13px;">${stationName}</strong><br/>
                  <span style="font-size: 11px; color: #666;">BSP Station • ${meters.length} meters</span><br/>
                  <div style="margin-top: 6px; font-size: 11px; max-height: 150px; overflow-y: auto;">
                    ${metersList}
                  </div>
                </div>`
              )
              .addTo(mapInstance);
            
            console.log("[v0] Popup added to map");
          });

          markersRef.current.push(marker);
        });

        // Add individual meter markers
        individualMeters.forEach((meter) => {
          if (!map.current) return;

          const el = document.createElement("div");
          el.style.cssText = "width: 12px; height: 12px; cursor: pointer;";

          const inner = document.createElement("div");
          let color = "#64748b";
          if (meter.type === "DTX") color = "#a855f7";
          else if (meter.type === "REGIONAL_BOUNDARY") color = "#10b981";
          else if (meter.type === "DISTRICT_BOUNDARY") color = "#f97316";
          else if (meter.type === "BSP") color = "#3b82f6";

          inner.style.cssText = `
                        width: 100%;
                        height: 100%;
                        background-color: ${color};
                        border: 2px solid white;
                        border-radius: 50%;
                        box-shadow: 0 1px 4px rgba(0,0,0,0.3);
                        transition: transform 0.2s ease;
                    `;
          el.appendChild(inner);

          el.addEventListener("mouseenter", () => {
            inner.style.transform = "scale(1.3)";
            inner.style.boxShadow = "0 2px 6px rgba(0,0,0,0.4)";
          });

          el.addEventListener("mouseleave", () => {
            inner.style.transform = "scale(1)";
            inner.style.boxShadow = "0 1px 4px rgba(0,0,0,0.3)";
          });

          const marker = new maplibregl.Marker({
            element: el,
            anchor: "center",
          })
            .setLngLat([meter.lng, meter.lat])
            .addTo(map.current);

          // Add click handler to show shared popup
          el.addEventListener("click", (e) => {
            e.stopPropagation();
            const mapInstance = map.current;
            if (!mapInstance) return;
            
            // Remove existing popup if any
            if (popupRef.current) {
              popupRef.current.remove();
            }

            // Create new popup at this meter's location
            popupRef.current = new maplibregl.Popup({ 
              offset: 10,
              closeButton: true,
              closeOnClick: true 
            })
              .setLngLat([meter.lng, meter.lat])
              .setHTML(
                `<div style="padding: 6px;">
                  <strong style="font-size: 11px;">${meter.meter_number}</strong><br/>
                  <span style="font-size: 10px; color: #666;">Type: ${meter.type}</span><br/>
                  ${meter.brand ? `<span style="font-size: 10px; color: #666;">Brand: ${meter.brand}</span><br/>` : ""}
                  ${meter.station ? `<span style="font-size: 10px; color: #666;">Station: ${meter.station}</span><br/>` : ""}
                  ${meter.boundary_metering_point ? `<span style="font-size: 10px; color: #666;">Boundary: ${meter.boundary_metering_point}</span>` : ""}
                </div>`
              )
              .addTo(mapInstance);
          });

          markersRef.current.push(marker);
        });

        // Fit bounds to include districts and meters
        const mapBounds = new maplibregl.LngLatBounds();

        districtGeometries.forEach((district) => {
          if (district.geometry && district.geometry.type === "Polygon") {
            district.geometry.coordinates[0].forEach((coord: number[]) => {
              mapBounds.extend(coord as [number, number]);
            });
          }
        });

        validMeters.forEach((meter) => {
          mapBounds.extend([meter.lng, meter.lat]);
        });

        map.current.fitBounds(mapBounds, { padding: 50 });
      } else {
        map.current.fitBounds(bounds, { padding: 40 });
      }

      // Trigger resize after map loads to ensure proper rendering
      setTimeout(() => {
        if (map.current) {
          map.current.resize();
        }
      }, 100);
    });

    return () => {
      if (popupRef.current) {
        popupRef.current.remove();
        popupRef.current = null;
      }
      markersRef.current.forEach((marker) => marker.remove());
      markersRef.current = [];
      map.current?.remove();
      map.current = null;
    };
  }, [districtGeometries, regionName, meterCoordinates]);

  const bspCount = meterCoordinates.filter((m) => m.type === "BSP").length;
  const dtxCount = meterCoordinates.filter((m) => m.type === "DTX").length;
  const boundaryCount = meterCoordinates.filter(
    (m) => m.type === "REGIONAL_BOUNDARY" || m.type === "DISTRICT_BOUNDARY",
  ).length;

  return (
    <Card className="h-full flex flex-col">
      <CardHeader className="pb-2 flex-shrink-0">
        <CardTitle className="text-base">Region Map</CardTitle>
        <CardDescription className="text-xs">
          District boundaries visualization
        </CardDescription>
      </CardHeader>
      <CardContent className="p-0 flex-1 flex flex-col">
        {districtGeometries.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-muted-foreground text-sm">
            No district boundaries available
          </div>
        ) : (
          <>
            <div ref={mapContainer} className="flex-1 w-full min-h-[450px] relative">
              {/* District hover info overlay */}
              {hoveredDistrict && (
                <div className="absolute top-3 left-3 bg-background/95 backdrop-blur-sm border rounded-lg shadow-lg p-3 z-10 min-w-[200px]">
                  <div className="font-semibold text-sm">{hoveredDistrict.name}</div>
                  {hoveredDistrict.consumption !== undefined && (
                    <div className="text-xs text-muted-foreground mt-1">
                      Consumption: {hoveredDistrict.consumption.toLocaleString()} kWh
                    </div>
                  )}
                </div>
              )}
            </div>
            
            <div className="flex-shrink-0 p-3 mx-3 mb-3 flex items-start gap-2 text-xs text-muted-foreground bg-muted/30 rounded">
              <MapPin className="h-3.5 w-3.5 mt-0.5 shrink-0" />
              <div className="flex-1">
                <p className="font-medium">{regionName}</p>
                <p>{districtGeometries.length} districts</p>
                {meterCoordinates.length > 0 && (
                  <p className="mt-1">
                    {bspCount > 0 && (
                      <>
                        <span className="inline-block w-2 h-2 rounded-full bg-blue-500 border border-white mr-1" />
                        {bspCount} BSP
                      </>
                    )}
                    {dtxCount > 0 && (
                      <>
                        {bspCount > 0 && " • "}
                        <span className="inline-block w-2 h-2 rounded-full bg-purple-500 border border-white mr-1" />
                        {dtxCount} DTX
                      </>
                    )}
                    {boundaryCount > 0 && (
                      <>
                        {(bspCount > 0 || dtxCount > 0) && " • "}
                        <span className="inline-block w-2 h-2 rounded-full bg-green-500 border border-white mr-1" />
                        {boundaryCount} boundary
                      </>
                    )}
                  </p>
                )}
              </div>
            </div>
          </>
        )}
      </CardContent>
    </Card>
  );
}
